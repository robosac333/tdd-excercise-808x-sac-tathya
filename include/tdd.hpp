...
/**
 * @class PIDController
 * @brief Inherits getter and setter methods from GetterSetter and implements
 * PID control logic.
 */
class PIDController {
 private:
 ...
 public:
  ...
  PIDController(double Kp, double Ki, double Kd, double setpoint,
                double current_value, double dt, double prev_error,
  ...
  /**
   * @brief Updates the PID controller for one time step and computes the
   * output.
