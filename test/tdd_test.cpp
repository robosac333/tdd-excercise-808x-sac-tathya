...
TEST(PIDControllerTest, IntegralControl) {
  ...
  double error = -10;
  EXPECT_DOUBLE_EQ(pid.IntControl(error),
                   -0.6);  // Expect near due to floating-point precision
}
...
TEST(PIDControllerTest, DerivativeControl) {
  ...
  EXPECT_DOUBLE_EQ(
      pid.DerControl(error),
      -30.0);  // Derivative gain Kd = 3, dt = 0.1, error change = -10
}
...
TEST(PIDControllerTest, CalculateOutput) {
  ...
  double error = -10;
  EXPECT_DOUBLE_EQ(pid.calcOutput(error),
                   -38.6);  // Output should be -8 + (-0.06) + (-30) = -38.606
}
